...
type
  ...
  { TSpaceActor }
   ...
   TImpulseLazer = class(TSpaceActor)
     private
       FLifeTime: Single;
...
begin
  ...
  // Снова фиксируем позиции статических объектов после обработки столкновений
  for i := 0 to FActorList.Count - 1 do
  begin
    if (TSpaceActor(FActorList.Items[i]).MaxSpeed = 0) or
       (TSpaceActor(FActorList.Items[i]).ShipType = stPlanet) then
...
begin
  ...
  if (IsKeyDown(KEY_LEFT_SHIFT)) then Ship.Shot;
...
var
  ...
  ImpactForce: Single;
  IsThisStatic, IsOtherStatic: Boolean;
const
  Elasticity = 0.3;
  MinImpactForce = 0.1;
...
begin
  ...
  begin
    // Получаем коррекцию столкновения
    Correction := GetCollisionCorrection(@FCollider, @Other.FCollider);
    CollisionNormal := Vector3Normalize(Correction);

    // ОБРАБОТКА СТОЛКНОВЕНИЙ В ЗАВИСИМОСТИ ОТ ТИПОВ ОБЪЕКТОВ
    if IsThisStatic and IsOtherStatic then
    begin
      // Два статических объекта - ничего не делаем
      Exit;
    end
    else if IsThisStatic then
    begin
      // Этот объект статический, другой движется - сдвигаем только движущийся
      Other.FPosition := Vector3Add(Other.FPosition, Correction);
      SetColliderTranslation(@Other.FCollider, Other.FPosition);
    end
    else if IsOtherStatic then
    begin
      // Этот объект движется, другой статический - сдвигаем только движущийся
      FPosition := Vector3Add(FPosition, Correction);
      SetColliderTranslation(@FCollider, FPosition);
    end
    else
    begin
      // Оба объекта движутся - сдвигаем оба
      FPosition := Vector3Add(FPosition, Vector3Scale(Correction, 0.5));
      Other.FPosition := Vector3Add(Other.FPosition, Vector3Scale(Correction, -0.5));
      SetColliderTranslation(@FCollider, FPosition);
      SetColliderTranslation(@Other.FCollider, Other.FPosition);
    end;

    // Расчет относительной скорости (только для движущихся объектов)
    if not IsThisStatic and not IsOtherStatic then
    begin
      ...
      begin
        ...
        Other.FCurrentSpeed := Other.FCurrentSpeed * (1.0 - Min(ImpactForce/Other.MaxSpeed, SpeedReductionFactor));

        // Эффект отскока
        FVelocity := Vector3Add(FVelocity, Vector3Scale(CollisionNormal, ImpactForce * Elasticity));
        Other.FVelocity := Vector3Add(Other.FVelocity, Vector3Scale(CollisionNormal, -ImpactForce * Elasticity));
      end;
    end
    else if not IsThisStatic and IsOtherStatic then
    begin
      // Движущийся объект ударяется о статический
      ImpactForce := Vector3DotProduct(FVelocity, CollisionNormal);

      if ImpactForce > MinImpactForce then
      begin
        FCurrentSpeed := FCurrentSpeed * (1.0 - Min(ImpactForce/MaxSpeed, SpeedReductionFactor));
        // Отскок от статического объекта
        FVelocity := Vector3Add(FVelocity, Vector3Scale(CollisionNormal, ImpactForce * Elasticity * 2));
      end;
    end
    else if IsThisStatic and not IsOtherStatic then
    begin
      // Статический объект ударяется о движущийся
      ImpactForce := Vector3DotProduct(Other.FVelocity, CollisionNormal);

      if ImpactForce > MinImpactForce then
      begin
        Other.FCurrentSpeed := Other.FCurrentSpeed * (1.0 - Min(ImpactForce/Other.MaxSpeed, SpeedReductionFactor));
        // Отскок от статического объекта
        Other.FVelocity := Vector3Add(Other.FVelocity, Vector3Scale(CollisionNormal, -ImpactForce * Elasticity * 2));
      end;
    end;
...
begin
  ...
  begin
    ...
    SetColliderTranslation(@FCollider, FOriginalPosition);
  end;
end;
